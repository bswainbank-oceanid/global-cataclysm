...
PLAY = textwrap.dedent('''
    ...
    from engine.bots.driver import play_to_completion
    from server.lobby import build_session
    seat = {{'mode': 'BOT', 'faction': 'random', 'alliance': 0, 'strategy': 'random', 'behavior': 'random'}}
    session, _ = build_session({{'seats': [dict(seat) for _ in range(6)], 'seed': {seed}}})
...
def play_in_a_fresh_interpreter(seed, hash_seed, turns=40):
    code = PLAY.format(root=ROOT, seed=seed, turns=turns)
