...
Run: python tools/client_shot.py OUT.png [--cam x,y,zoom] [--hover ID]
         ...
         [--server [URL]] [--autoplay N] [--state FILE.json] [--badges flag|strip|category] [--wait FRAMES] [--size WxH]
"""
import argparse
import shutil
import subprocess
...
def main():
    ...
    for key in ('cam', 'hover', 'select', 'wheel', 'drag', 'click', 'state', 'badges', 'server', 'autoplay'):
        if getattr(a, key):
            val = getattr(a, key)
