...
def excluded_naval_purchase_zones(data_module):
    ...
    The map's naval_deploy_excluded sea zones -- the same zones that
    never receive a naval deployment in any starting setup (e.g. the
    landlocked Caspian Sea, id 43, per setup.excluded_naval_zones_note). The underlying reason (no real
    ...
    need to model)."""
    return set(data_module.naval_deploy_excluded())
